...
#        diag = expand("/global/scratch/users/arphillips/reports/filtering/wgs_aspen.all.genos.{region}.table", region = REGION),
#        dp_table = expand("/global/scratch/users/arphillips/reports/filtering/depth/wgs_aspen.all.genos.{chr}.filtered.nocall.table", chr = REGION),
#        filt_vcf = expand("/global/scratch/users/arphillips/data/processed/filtered_snps/wgs_aspen.all.genos.{region}.nocall01.{min_dp}dp{max_dp}.vcf.gz", region = REGION, min_dp = MIN_DP, max_dp = MAX_DP)
#        bcftools_concat = expand("/global/scratch/users/arphillips/data/processed/filtered_snps/wgs_aspen.all.genos.nocall01.{min_dp}dp{max_dp}.vcf.gz", min_dp = MIN_DP, max_dp = MAX_DP)
